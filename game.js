// ============================================================
// GAME.JS - BİLARDO / MASA FUTBOLU OYUNU
// ============================================================
...
// --- RESİM / ASSET YÖNETİMİ ---
const images = {};
const playerFormas = {
    team1: 'menu/ayarlar/forma/f1.webp',
    team2: 'menu/ayarlar/forma/f2.webp'
};

// Varsayılan stadyum texture yolu ve Saha Görseli Yönetimi
...
// Oyuncu formalarını yükleme
function loadPlayerImages() {
    ['team1', 'team2'].forEach(team => {
        const img = new Image();
        img.src = playerFormas[team];
        img.onload = () => {
            images[team] = img;
            console.log(`✅ ${team} forması yüklendi: ${playerFormas[team]}`);
        };
        img.onerror = () => {
            console.error(`❌ ${team} forması yüklenemedi: ${playerFormas[team]}`);
        };
    });
}

// Sayfa yüklendiğinde görselleri çek
loadFieldImage();
loadPlayerImages();

// --- OYUN AYARLARI VE DEĞİŞKENLERİ ---
let matchDurationSec = 180; // Varsayılan 3 dakika
let turnDurationSec = 15;   // Varsayılan 15 saniye

let gameTimer = null;
let turnTimer = null;
let matchTimeRemaining = matchDurationSec;
let turnTimeRemaining = turnDurationSec;

let currentPhase = 'menu'; // 'menu', 'formation', 'playing', 'gameover'
let activeTeam = 'team1';  // 'team1' (Ev Sahibi / Kırmızı) veya 'team2' (Deplasman / Mavi)
let team1Score = 0;
let team2Score = 0;
...
// Kale Alanları (Ölçeklenebilir)
let goals = {
    left: { x: 0, yMin: 180, yMax: 320 },
    right: { x: 800, yMin: 180, yMax: 320 }
};

// --- EKRAN / SAYFA YÖNETİMİ ---
...
function showField() {
    ...
    if (gameCanvas) {
        ...
        gameCanvas.style.border = '4px solid rgba(27, 94, 32, 0.4)';
        gameCanvas.style.borderRadius = '8px';
        gameCanvas.style.boxShadow = '0 10px 40px rgba(0, 0, 0, 0.6)';
...
function startGame() {
    ...
    // Skor ve Süre Sıfırlama
    team1Score = 0;
    team2Score = 0;
    matchTimeRemaining = matchDurationSec;
    turnTimeRemaining = turnDurationSec;
    activeTeam = 'team1';
...
function resetPositions() {
    ...
    // Oyuncu Dizilişleri (5v5 Örnek)
    players = [
        // Team 1 (Ev Sahibi - Sol)
        { id: 1, team: 'team1', x: 60,  y: 250, vx: 0, vy: 0, radius: 18, isGK: true },
        { id: 2, team: 'team1', x: 200, y: 120, vx: 0, vy: 0, radius: 18 },
        { id: 3, team: 'team1', x: 200, y: 380, vx: 0, vy: 0, radius: 18 },
        { id: 4, team: 'team1', x: 320, y: 200, vx: 0, vy: 0, radius: 18 },
        { id: 5, team: 'team1', x: 320, y: 300, vx: 0, vy: 0, radius: 18 },

        // Team 2 (Deplasman - Sağ)
        { id: 6, team: 'team2', x: 740, y: 250, vx: 0, vy: 0, radius: 18, isGK: true },
        { id: 7, team: 'team2', x: 600, y: 120, vx: 0, vy: 0, radius: 18 },
        { id: 8, team: 'team2', x: 600, y: 380, vx: 0, vy: 0, radius: 18 },
        { id: 9, team: 'team2', x: 480, y: 200, vx: 0, vy: 0, radius: 18 },
        { id: 10, team: 'team2', x: 480, y: 300, vx: 0, vy: 0, radius: 18 }
    ];
}
...
function resetTurnTimer() {
    ...
    turnTimer = setInterval(() => {
        if (turnTimeRemaining > 0) {
            ...
            switchTurn(); // Süre bitti, sıra karşı tarafa geçer
        }
    }, 1000);
}
...
function switchTurn() {
    ...
    resetTurnTimer();
    console.log(`🔄 Sıra Değişti: ${activeTeam}`);
}

function updateScoreBoard() {
    const t1ScoreEl = document.getElementById('team1-score');
    const t2ScoreEl = document.getElementById('team2-score');
    if (t1ScoreEl) t1ScoreEl.innerText = team1Score;
    if (t2ScoreEl) t2ScoreEl.innerText = team2Score;
}
...
function updateTimerDisplay() {
    ...
    if (timerEl) {
        ...
        const secs = matchTimeRemaining % 60;
        timerEl.innerText = `${mins}:${secs < 10 ? '0' : ''}${secs}`;
    }
}

function updateTurnTimerDisplay() {
    const turnEl = document.getElementById('turn-timer');
    if (turnEl) {
        turnEl.innerText = turnTimeRemaining;
    }
}
...
function endGame() {
    ...
    alert(`Maç Bitti!\nSkor: ${team1Score} - ${team2Score}`);
    backToMenu();
}

// --- FİZİK VE GÜNCELLEME MOTORU ---
function updatePhysics() {
    ...
    // Top Duvar Çarpmaları
    if (ball.x - ball.radius < 0) {
        if (ball.y > goals.left.yMin && ball.y < goals.left.yMax) {
            goalScored('team2'); // Deplasman gol attı
            return;
        } else {
            ball.x = ball.radius;
            ball.vx *= -BOUNCE;
    ...
    if (ball.x + ball.radius > canvas.width) {
        if (ball.y > goals.right.yMin && ball.y < goals.right.yMax) {
            goalScored('team1'); // Ev sahibi gol attı
            return;
        } else {
    ...
    // Oyuncuların Hareketi ve Çarpışmaları
    players.forEach(p => {
        p.x += p.vx;
        p.y += p.vy;
        p.vx *= FRICTION;
        ...
        if (p.x - p.radius < 0) { p.x = p.radius; p.vx *= -BOUNCE; }
        if (p.x + p.radius > canvas.width) { p.x = canvas.width - p.radius; p.vx *= -BOUNCE; }
        if (p.y - p.radius < 0) { p.y = p.radius; p.vy *= -BOUNCE; }
        if (p.y + p.radius > canvas.height) { p.y = canvas.height - p.radius; p.vy *= -BOUNCE; }
...
function goalScored(scoringTeam) {
    if (scoringTeam === 'team1') team1Score++;
    else team2Score++;

    updateScoreBoard();
    alert(`GOL! ${scoringTeam === 'team1' ? 'Ev Sahibi' : 'Deplasman'} Kazandı!`);
    resetPositions();
}
...
function draw() {
    ...
    ctx.lineWidth = 3;
    ctx.strokeRect(0, goals.left.yMin, 20, goals.left.yMax - goals.left.yMin);
    ctx.strokeRect(canvas.width - 20, goals.right.yMin, 20, goals.right.yMax - goals.right.yMin);
    ...
    players.forEach(p => {
        ...
        if (images[p.team]) {
            ctx.drawImage(images[p.team], p.x - p.radius, p.y - p.radius, p.radius * 2, p.radius * 2);
        } else {
            ctx.fillStyle = p.team === 'team1' ? '#e53935' : '#1e88e5';
            ctx.fill();
        }
        ctx.restore();

        // Aktif Sıra / Seçim Çerçevesi
...
// --- İNTERAKTİF NİŞAN ALMA VE ATIŞ MANTIĞI ---
...
canvas.addEventListener('mouseup', () => {
    if (!isAiming || !selectedPlayer) return;

    // Vektör Hesabı (Ters Çekip Fırlatma Mantığı)
    let dx = aimStart.x - aimCurrent.x;
    let dy = aimStart.y - aimCurrent.y;

    let power = Math.min(Math.hypot(dx, dy) / 10, MAX_POWER);
    let angle = Math.atan2(dy, dx);

    selectedPlayer.vx = Math.cos(angle) * power;
    selectedPlayer.vy = Math.sin(angle) * power;

    isAiming = false;
    selectedPlayer = null;

    // Atış yapıldıktan sonra sırayı değiştir
    setTimeout(() => {
        switchTurn();
    }, 1500);
});
...
// ============================================================
// SETTINGS / AYARLAR SEÇİM MANTIKLARI
// ============================================================

// --- FORMA SEÇİMİ ---
function toggleJerseyOptions(team) {
    const optionsDiv = document.getElementById(`${team}-jersey-options`);
    if (!optionsDiv) return;

    const otherTeam = team === 'team1' ? 'team2' : 'team1';
    const otherDiv = document.getElementById(`${otherTeam}-jersey-options`);
...
function selectJersey(team, jerseyImgName) {
    const fullPath = `menu/ayarlar/forma/${jerseyImgName}`;
    playerFormas[team] = fullPath;

    const previewImg = document.getElementById(`${team}-jersey-preview`);
    if (previewImg) previewImg.src = fullPath;

    const cards = document.querySelectorAll(`#${team}-jersey-options .jersey-option-card`);
    cards.forEach(card => {
        const img = card.querySelector('img');
        if (img && img.src.includes(jerseyImgName)) {
...
function toggleMatchDurationOptions() {
    const optionsDiv = document.getElementById('match-duration-options');
    if (!optionsDiv) return;
    optionsDiv.style.display = (optionsDiv.style.display === 'none' || optionsDiv.style.display === '') ? 'flex' : 'none';
}
...
function selectMatchDuration(minutes, element) {
    ...
    const selectedText = document.getElementById('selected-match-duration');
    if (selectedText) selectedText.innerText = `${minutes} Dk`;

    const cards = document.querySelectorAll('#match-duration-options .setting-option-card');
...
function selectShotDuration(seconds, element) {
    turnDurationSec = seconds;
    const selectedText = document.getElementById('selected-shot-duration');
    if (selectedText) selectedText.innerText = `${seconds} Sn`;

    const cards = document.querySelectorAll('#shot-duration-options .setting-option-card');
    cards.forEach(c => c.classList.remove('active'));
    if (element) element.classList.add('active');
...
// Zemin seçme fonksiyonu (HTML'den tetiklenir)
function selectStadium(stadiumKey, texturePath) {
    // 1. Oyun alanında kullanılacak `-t` ekli texture yolunu kaydet
...
console.log('⏱️ Varsayılan maç süresi:', window.MATCH_DURATION, 'sn');
// ============================================================
// AYARLAR - MAÇ SÜRESİ (DOSYA SONUNA EKLE)
// ============================================================
