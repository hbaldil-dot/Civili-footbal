...
// ============================================================
// IOS SAFARI UYUMLULUK - CANVAS BOYUTLANDIRMA
// ============================================================

function setupCanvasForiOS() {
    var canvas = document.getElementById('gameCanvas');
    var container = document.querySelector('.canvas-container');
    
    if (!canvas || !container) return;
    
    // iOS Safari'de safe-area'ları kontrol et
    var isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
    
    if (isIOS) {
        console.log('📱 iOS Safari tespit edildi - Canvas ayarlanıyor...');
        
        // Container yüksekliğini ayarla
        var vh = window.innerHeight;
        var topBarHeight = document.getElementById('top-bar').offsetHeight || 60;
        var powerBarHeight = document.getElementById('power-bar-container').offsetHeight || 20;
        var availableHeight = vh - topBarHeight - powerBarHeight - 40;
        ...
        console.log('📱 iOS Canvas boyutu:', rect.width, 'x', rect.height);
    }
}
...
// Pencere yeniden boyutlandığında da çalıştır
window.addEventListener('resize', function() {
    setupCanvasForiOS();
});
// ============================================================
// SAHA SINIRLARINI KONTROL ET (iOS İÇİN)
// ============================================================
...
function checkFieldBoundaries() {
    ...
    console.log('🏟️ SAHA SINIRLARI:');
    console.log('  Genişlik:', width, 'Yükseklik:', height);
    console.log('  Kale sol:', goalLeft, 'Kale sağ:', goalRight, 'Kale Y:', goalY);
    console.log('  Sol duvar: 0, Sağ duvar:', width);
    console.log('  Üst duvar: 0, Alt duvar:', height);
    
    // Topun sınırlar içinde olduğundan emin ol
    if (cap.x < 0) cap.x = 0;
    if (cap.x > width) cap.x = width;
    if (cap.y < 0) cap.y = 0;
    if (cap.y > height) cap.y = height;
    
    // Pinlerin sınırlar içinde olduğundan emin ol
...
// ============================================================
// DUVAR SEKTİRME - iOS SAFARI TESTİ
// ============================================================

function testWallShotIOS() {
    console.log('🧪 iOS DUVAR TESTİ BAŞLATILIYOR...');
    console.log('📍 Top konumu:', cap.x, cap.y);
    console.log('📐 Canvas boyutu:', canvas.width, 'x', canvas.height);
    console.log('📐 Canvas görünür boyut:', canvas.clientWidth, 'x', canvas.clientHeight);
    
    // Top hangi duvara daha yakın?
    var distToLeft = cap.x;
    var distToRight = width - cap.x;
    var distToTop = cap.y;
    var distToBottom = height - cap.y;
    
    console.log('📏 Duvara mesafeler - Sol:', distToLeft, 'Sağ:', distToRight, 'Üst:', distToTop, 'Alt:', distToBottom);
    
    // En yakın duvarı bul
    var nearestWall = 'bottom';
    var nearestDist = distToBottom;
    
    if (distToLeft < nearestDist) { nearestWall = 'left'; nearestDist = distToLeft; }
    if (distToRight < nearestDist) { nearestWall = 'right'; nearestDist = distToRight; }
    if (distToTop < nearestDist) { nearestWall = 'top'; nearestDist = distToTop; }
    
    console.log('🧱 EN YAKIN DUVAR:', nearestWall, 'Mesafe:', nearestDist);
    
    // Vuruş açısı ve gücü
    var angle, pullDistance;
    ...
    console.log('💪 VURUŞ - Açı:', angle * 180 / Math.PI, 'Güç:', pullDistance);
    ...
    console.log('✅ DUVAR TESTİ TAMAMLANDI - Hedef:', nearestWall);
}
